...
/// </summary>
internal abstract class EyeXGameViewBoundsProvider
{
    protected IntPtr _mainWindowWindowHandle;
    protected IntPtr _gameViewWindowHandle;

    /// <summary>
    /// Initializes a new instance of the <see cref="EyeXGameViewBoundsProvider"/> class.
    /// </summary>
    protected EyeXGameViewBoundsProvider()
    {
        _mainWindowWindowHandle = FindWindowWithThreadProcessId();
        _gameViewWindowHandle = GetGameViewWindowHandle();
        MainWindowId = _mainWindowWindowHandle.ToString();
    }

    /// <summary>
    /// Gets the Window ID for the game window.
    /// </summary>
    public string MainWindowId { get; private set; }
    ...
    /// <summary>
    /// Gets the window handle for the game view. This might not be a top level window.
    /// </summary>
    /// <returns></returns>
    protected virtual IntPtr GetGameViewWindowHandle()
    {
        return _mainWindowWindowHandle;
    }
...
/// </summary>
internal class UnityPlayerGameViewBoundsProvider : EyeXGameViewBoundsProvider
{
    protected override Rect GetGameViewLogicalBounds()
    {
        var topLeft = GetWindowPosition();
        var bottomRight = GetWindowBottomRight();
        if (float.IsNaN(topLeft.x) || float.IsNaN(bottomRight.x))
        {
        ...
        {
            //Full screen with abnormal settings (aspect ratio or scaling), unity always seems to scale to the physical screen
...
/// </summary>
internal class LegacyEditorGameViewBoundsProvider : EyeXGameViewBoundsProvider
{
...
/// </summary>
internal class EditorGameViewBoundsProvider : EyeXGameViewBoundsProvider
{
